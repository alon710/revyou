 ...
 */
export async function getReviewAdmin(reviewId: string): Promise<Review | null> {
  try {
...
export async function updateReviewReplyAdmin(
  ...
  try {
    ...
    return (await getReviewAdmin(reviewId)) as Review;
  } catch (error) {
 ...
 * @returns Updated review
 */
export async function markAsFailedAdmin(reviewId: string): Promise<Review> {
  try {
    const reviewRef = adminDb.collection("reviews").doc(reviewId);
